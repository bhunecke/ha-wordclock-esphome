...
void Wordclock::setup() {
  ...
  this->effect_.set_time(this->time_);
  this->light_->add_effects({&this->effect_});
  this->effect_.init_internal(this->light_);
  this->light_->make_call().set_effect("Wordclock Time").perform();
}
...
void WordclockEffect::apply(light::AddressableLight &parent, const Color &current_color) {
  if (time_ == nullptr) return;

  auto time = time_->now();
  if (!time.is_valid()) {
    parent[0] = Color(255, 0, 0, 0);
    parent.schedule_show();
    return;
  }
  ...
  Color off(0, 0, 0, 0);
  for (int i = 0; i < n; i++) {
    if (i < 110 || i > 120) parent[i] = off;
  }
  for (int i = 0; i < 5; i++) {
    parent[leds_time_it_is[i]] = current_color;
  }
  for (int i = 0; i < 15; i++) {
    if (leds_time_minutes[tmp_minute][i] >= 0) {
      parent[leds_time_minutes[tmp_minute][i]] = current_color;
    }
  }
  for (int i = 0; i < 6; i++) {
    int led_idx = leds_time_hours[tmp_hour][i];
    if (tmp_hour == 1 && tmp_minute == 0 && led_idx == 60) continue;
    if (led_idx >= 0) parent[led_idx] = current_color;
  }
  ...
  if (m != this->last_log_minute_) {
    ...
    ESP_LOGD(TAG, "Time: %i:%i  RGBW: %i-%i-%i-%i  (tmp_hour: %i tmp_minute: %i)",
             h, m, current_color.r, current_color.g, current_color.b, current_color.w,
             tmp_hour, tmp_minute);
